...
function begin_play(){
	var checkP1 = document.getElementById("player1_id");
	var checkP2 = document.getElementById("player2_id");
	var turnI = document.getElementById("turn_info");

	if (game_started()){alert("Already started. Please reset_play to start again.");}
	
	if (isEmpty(checkP1.value) || isEmpty(checkP2.value)){alert("Two player game, both the fields are mandatory.");}
	else
	{
		if (checkP1.disabled==0 && checkP2.disabled==0)
		{
			checkP1.value = checkP1.value + "(X)";
			checkP2.value = checkP2.value + "(O)";
		}
		checkP1.disabled = true;
		checkP2.disabled = true;
