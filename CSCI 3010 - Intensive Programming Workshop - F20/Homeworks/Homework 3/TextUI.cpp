...
/**
 * Function handles election type and rerouting user inputs
*/ 
...
void TextUI::CampaignUI(Election &election, ElectoralMap &electoral){
    ...
    while (campaing_candidate != 0){
        cand_found = 0;
        std::cout << "Which candidate is campaigning (id) (0 to stop campaigning)? "; //handles which candidate
        ...
        campaing_district = -1;
        while (campaing_district != 0){ //handles where
            dist_found = 0;
