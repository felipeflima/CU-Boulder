...
void Election::RegisterCandidates(std::string candidate_name, int party_num, int candidate_id){
    ...
    switch (party_num) //add party affiliation depending on param
    {
    case 1:
...
/**
 * Function calculates and rearenges constituents based on the write up
 * @param a district
*/ 
...
Candidate Election::WinnerInDistrict(District d){
    ...
    for (it_v = votes_.begin(); it_v != votes_.end(); it_v++){
        candidate_in_party = 0;
        if (it_v->first == Party::None){ //
            votes_[GetPartyMostConstituentsVote(Party::None)] += votes_[Party::None]; //tranfer all the votes from Party::None to the one with most constituents
            votes_[Party::None] = 0;
        }
        else{
            for (it_c = candidates_.begin(); it_c != candidates_.end(); it_c++){ //check for candidate in the party
                if (it_v->first == it_c->second.party_affiliation){
                    candidate_in_party = 1;
                }
            }
            if (candidate_in_party == 0){ // if there are no candidates in the party
                votes_[GetPartyMostConstituentsVote(it_v->first)] += votes_[it_v->first]; //tranfer all the votes from that party to the one with most constituents
                votes_[it_v->first] = 0;
            }
        }
    }
    for (it_v = votes_.begin(); it_v != votes_.end(); it_v++){ //for every party
        for (it_c = candidates_.begin(); it_c != candidates_.end(); it_c++){ //for every candidate
            if (it_v->first == it_c->second.party_affiliation && it_v->second > most){// if the current party and the party from the candidate match and the votes for that party are greater than most
                if (it_c->second.votes == winner.votes && it_cmost->second.id != it_c->second.id && it_cmost->second.print == 1 && it_c->second.print == 1){
                random = (rand() % 2 + 1); // if there is more than one candidate in the party randomly choose one
                    if (random == 1){
                        it_cmost->second.print = 0;
                    }
                    else{
                ...
                if (it_c->second.print == 1){
                    winner = it_c->second; // change winner 
                    winner.votes = d.GetVotesDistrict(); // set the districts votes
                    most = it_v->second;
                }
            }
        }
    }
    std::cout << d.GetDistrictName() << ": " << winner.name <<  " with " << d.GetVotesDistrict() << " electoral votes!" << std::endl;
    return winner;
}