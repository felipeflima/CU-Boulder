...
void ElectoralMap::Campaign(int cand_id, int dist_id, Election &election){
    ...
    double const_none_party = districts_[dist_id].get_constituents()[Party::None];
    double const_cand_party = districts_[dist_id].get_constituents()[cand.party_affiliation];
    double const_other_party = districts_[dist_id].GetTotalConstituents() - const_none_party - const_cand_party;
    Party most_const = districts_[dist_id].GetPartyMostConstituents(cand.party_affiliation);
    double dist_area = districts_[dist_id].get_area();
    double P_success = (((const_cand_party+1)*2) / const_other_party) * (((const_cand_party+1)*2) / dist_area);
    if (P_success >= 1){
        P_success = 100.0;
    }
    else{
    ...
    if (P_success_rand <= P_success){
        if (districts_[dist_id].get_constituents()[Party::None] > 0){
        ...
        if (P_success_rand <= P_extra && districts_[dist_id].get_constituents()[most_const] > 0){
